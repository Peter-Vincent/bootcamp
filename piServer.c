...
int main(int argc, char *argv[]){
    int sockfd, newsockfd, primary_sock, portno, piservnumber, clilen, n, pid;
    char buffer[256];
    ...
    */
    ...
    fprintf(stdout,"Waiting for first connection...\n");
    primary_sock = accept(sockfd, (struct sockaddr *) &cli_addr, (socklen_t*) &clilen);
    if (primary_sock < 0) error((const char*)"Error connecting to primary client");
    fprintf(stdout,"Primary connection made on  socket #%d\n",primary_sock);
    ...
    while (1) {
        ...
        if (pid == 0) {
            close(sockfd);
            make_comm(newsockfd,primary_sock);
            exit(0);
        }
...
void make_comm(int sock,int primary){
    int n;
    char buffer[256];
    bzero(buffer,256);
    bool live = true;
    fprintf(stdout,"Recieving commands \n");
    while (live){
        n = read(sock,buffer,255);
        if (n < 0) error("ERROR reading from socket  ");
        n = write(primary,buffer,strlen(buffer));
        if (n < 0) error("ERROR writing to primary socket");
